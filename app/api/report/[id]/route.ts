...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pengajuan = await prisma.pengajuan.findUnique({ where: { id }, include: { officials: { orderBy: { sortOrder: 'asc' } }, products: true, ingredients: true, assignments: { include: { auditor: { select: { name: true } } } }, auditResults: true, sjphResponses: { include: { criterion: { include: { category: true } }, evidences: true } }, temuan: { include: { fixes: true } } } });
