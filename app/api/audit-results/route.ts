...
export async function POST(req: Request) {
  ...
  if (Array.isArray(body.results)) {
    ...
    await prisma.auditResult.upsert({
      where: { pengajuanId_section_criterion: { pengajuanId: body.pengajuanId, section: body.section, criterion: results[0].criterion } },
      update: { result: results[0].result, note: results[0].note?.trim() || null, auditorId: user.id },
      create: { pengajuanId: body.pengajuanId, section: body.section, criterion: results[0].criterion, result: results[0].result, note: results[0].note?.trim() || null, auditorId: user.id },
    });
    await prisma.auditLog.create({ data: { pengajuanId: body.pengajuanId, actorId: user.id, action: 'AUDIT_RESULT_UPDATED', description: `Hasil audit ${body.section} diperbarui.` } });
  }
