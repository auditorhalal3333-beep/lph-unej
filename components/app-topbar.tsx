'use client';
import { Bell, Menu, Search, ChevronDown, CheckCheck, ShieldCheck } from 'lucide-react';
...
export function AppTopbar({ onMenu }: { onMenu?: () => void }) {
  ...
  const isAdminAccount = identity.role === 'ADMIN' || identity.role === 'SUPER_ADMIN';
  const isAuditorWorkspace = pathname.startsWith('/auditor') || /\/pengajuan\/[^/]+\/audit$/.test(pathname);
  useEffect(() => {
    let mounted = true;
  ...
  useEffect(() => { const close = (event: MouseEvent) => { const target = event.target as Node; if (ref.current && !ref.current.contains(target)) setOpen(false); if (profileRef.current && !profileRef.current.contains(target)) setProfileOpen(false); }; document.addEventListener('mousedown', close); return () => document.removeEventListener('mousedown', close); }, []);
